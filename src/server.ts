...
app.post("/courses", async (request: Request, response: Response) => {
  ...
  await knex("courses").insert({ name }); // inserindo um novo curso na tabela "courses"
...
app.get("/courses", async (request: Request, response: Response) => {
  const courses = await knex("courses").select().orderBy("name"); // selecionando todos os cursos da tabela "courses"
...
app.delete("/courses/:id", async (request: Request, response: Response) => {
  ...
  await knex("courses").delete().where({ id }); // deletando o curso com o id especificado
